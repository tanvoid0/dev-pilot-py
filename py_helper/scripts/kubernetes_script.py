...
class KubernetesScript(OptionGroupModel):
    def __init__(self):
        super().__init__(
            "Kubernetes Commands",
            [
                OptionModel("kdg", "Get Deployments", "Find deployments", self.get_deployments),
                OptionModel("ksu", "Upscale in batch", "", self.scale_batch),
                OptionModel("ksd", "Downscale in batch", "", lambda: self.scale_batch(down=True)),
                ...
                OptionModel("kcg", "Get Kubernetes config", "", self.get_deployment_yaml_config),
                OptionModel("ksg", "Get Kubernetes config", "", self.get_yaml_service_config)
            ]
        )
    ...
    def get_deployments(self):
        ...
        data = DeploymentModel.statefulset_from_kubectl_command_string(output, config.namespace)
        output = Commander.execute(f"kubectl get deployments -n {config.namespace}")
        data += DeploymentModel.deployment_from_kubectl_command_string(output, config.namespace)
        ...
        ordered_data = DeploymentModel.order_by_selection_names(data, updated_deployments)
    ...
    def scale_batch(self, down=False):
        ...
        delay = config_file['kubernetes']['scale']['down'] if down else config_file['kubernetes']['scale']['up']
        batch_cmd = f" sleep {delay}s; ".join(cmd)
        Commander.execute_externally(batch_cmd)

    def get_deployment_yaml_config(self):
        ...
        yaml_config = Commander.execute(f"kubectl get deployment {deployment_name} -o yaml -n {config.namespace}",
                                        show=True)
        ...
        file_path = os.path.join(FileProcessor.current_path(), "kube-configs", deployment_name)
    ...
    def get_yaml_service_config(self):
        ...
        yaml_config = Commander.execute(f"kubectl get service {service_name} -o yaml -n {config.namespace}", show=True)
        yaml_config = yaml.safe_load(yaml_config)
        ## Copy Values
        ...
        file_path = os.path.join(FileProcessor.current_path(), "kube-configs", service_name)
        if not os.path.exists(file_path):
            os.mkdir(file_path)
